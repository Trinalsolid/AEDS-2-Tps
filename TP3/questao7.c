...
Character array[1000];   // Elementos da fila circular 
...
void remover() {
   ...
   Character resp = topo->elemento;
   printf("%s %s\n", "(R)", topo->elemento.name);
   Celula* tmp = topo;
   topo = topo->prox;
   tmp->prox = NULL;
   free(tmp);
...
char OpCode(char entrada[]){
    ...
    if (entrada[0] == 'I'){
        ...
        inserir(character_searchById(Characternalista));
    }
    // REMOCOES
    if (entrada[0] == 'R'){
        remover();
