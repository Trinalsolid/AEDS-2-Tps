...
typedef struct CelulaDupla{
    Character elemento;         // Elemento inserido na celula.
    struct CelulaDupla *prox; // Aponta a celula prox.
    struct CelulaDupla *ant;  // Aponta a celula anterior.
} CelulaDupla;

CelulaDupla *novaCelulaDupla(Character elemento){
...
void inserir(Character *x, int pos){
...
Character removerFim(){
    ...
    Character resp = ultimo->elemento;
    ultimo = ultimo->ant;
...
void quicksortRec(int esq, int dir){
    int i = esq, j = dir;
    Character pivo = characters[(dir + esq) / 2];
    Character temp;
    while (i <= j){
        while (strcmp(characters[i].house, pivo.house) < 0 || strcmp(characters[i].house, pivo.house) == 0 && strcmp(characters[i].name, pivo.name) < 0)
            i++;
        while (strcmp(characters[j].house, pivo.house) > 0 || strcmp(characters[j].house, pivo.house) == 0 && strcmp(characters[j].name, pivo.name) > 0)
            j--;
        if (i <= j){
            swap(j , i);
            i++;
...
void sort(){
    quicksortRec(0, charactersLength-1);
}
