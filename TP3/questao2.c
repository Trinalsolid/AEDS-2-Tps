...
//C:\\Users\\WazX\\Desktop\\aeds2-master\\tps\\entrada e saida\\tp02\\characters.csv
 ...
 */
void inserir(Character *x, char posicao[]) {
    int i;
    int pos = atoi(posicao);

    //validar insercao
    if(listatam >= MAXTAM || pos < 0 || pos > listatam){
        printf("Erro ao inserir!");
        exit(1);
    }

    //levar elementos para o fim do array
    for(i = listatam; i > pos; i--){
        lista.array[i] = lista.array[i-1];
    }
...
Character removerInicio() {
    ...
    resp = lista.array[0];
    printf("%s %s\n", "(R)", lista.array[0].name);
    listatam--;

    for(i = 0; i < listatam; i++){
        lista.array[i] = lista.array[i+1];
    }
...
Character removerFim(){
    ...
    printf("%s %s\n", "(R)", lista.array[listatam].name);
    return lista.array[--listatam];
}
 ...
 * @return resp int elemento a ser removido.
 */
Character remover(char posicao[]) {
    int i;
    int pos = atoi(posicao);
    Character resp;

    //validar remocao
    if (listatam == 0 || pos < 0 || pos >= listatam) {
        printf("Erro ao remover!");
        exit(1);
    }

    resp = lista.array[pos];
    printf("%s %s\n", "(R)", lista.array[pos].name);
    listatam--;

    for(i = pos; i < listatam; i++){
        lista.array[i] = lista.array[i+1];
    }
...
void imprimirLista(){
    for (int i = 0; i < listatam; i++) {
        printf("[%d ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s]\n",
            i,
            character_getId(&lista.array[i]),
            character_getName(&lista.array[i]),
            character_getAlternateNames(&lista.array[i]),
            character_getHouse(&lista.array[i]),
            character_getAncestry(&lista.array[i]),
            character_getSpecies(&lista.array[i]),
            character_getPatronus(&lista.array[i]),
            character_getHogwartsStaff(&lista.array[i]) ? "true" : "false",
            character_getHogwartsStudent(&lista.array[i]) ? "true" : "false",
            character_getActorName(&lista.array[i]),
            character_getAlive(&lista.array[i]) ? "true" : "false",
            character_getBirthDate(&lista.array[i]),
            character_getYearOfBirth(&lista.array[i]),
            character_getEyeColour(&lista.array[i]),
            character_getGender(&lista.array[i]),
            character_getHairColour(&lista.array[i]),
            character_getWizard(&lista.array[i]) ? "true" : "false"
        );
    }
}

// OPCODE DE OPERACOES DE INSERSCAO E REMOCAO
...
char OpCode(char entrada[]) {
    ...
    if (caso[0] == 'I') {
        if (caso[1] == 'I') {
            // Insere Início
            personagemLista = strtok(NULL, " ");
            inserirInicio(character_searchById(personagemLista));
        } else if (caso[1] == 'F') {
            // Insere Fim
            personagemLista = strtok(NULL, " ");
            inserirFim(character_searchById(personagemLista));
        } else {
            // Insere Posição
            posicao = strtok(NULL, " ");
            personagemLista = strtok(NULL, " ");
            inserir(character_searchById(personagemLista), posicao);
        }
    }else if (caso[0] == 'R') {
        if (caso[1] == 'I') {
            // Remove Início
...
int main() {
    ...
    while(true) {
        ...
        if(isEnd(id)) break;
            ...
            // If the character is found, insert it into the list
            if (characterToAdd) {
                inserirFim(characterToAdd);
            } else {
                printf("Character with ID %s not found.\n", id);
            }
    ...
    for (int i = 0; i < operacoes; i++) {
        ...
        // Clean \n from the end of the string
        if(entrada[strlen(entrada) - 1] == '\n' || entrada[strlen(entrada) - 1] == '\r') entrada[strlen(entrada) - 1] = '\0';
