...
#define FILE_PATH "C:\\Users\\WazX\\Desktop\\aeds2-master\\tps\\entrada e saida\\tp02\\characters.csv"
...
char *character_getBirthDate(Character *character) {
    ...
    if(character->birthDate.day != -1 && character->birthDate.month != -1 && character->birthDate.year != -1) {
        sprintf(birthDate, "%02d-%02d-%02d", character->birthDate.day, character->birthDate.month, character->birthDate.year);
    }
...
void character_setBirthDate(Character *character, int birthDate) { 
    ...
    Date data;
    sscanf(birthDate, "%d-%d-%d", &character->birthDate.year,&character->birthDate.month, &character->birthDate.day);
...
void character_print(Character *character , int pos) {
...
Fila inserir(Character *x, Fila fila) {
    Character temp[MAXTAM];

    if (quantFila >= MAXTAM){
        temp[0] = fila.array[0];
        soma = soma - temp[0].yearOfBirth;
        quantFila--;
    ...
    quantFila++;
    double media = 0;
    soma = soma + x->yearOfBirth;
    media = soma / quantFila;
    media = round(media);
    printf(">> Year Birthday Average: %.0f\n", media);
    return fila;
}
...
void mostrarfila(){
    printf("[ Head ]\n");
    for (int i = 0; i < quantFila; i++){
        character_print(&fila.array[i], i);
    }
    printf("[ Tail ]");
}

void imprimirRemover(Character x){
   printf("%s%s\n", "(R) ", x.name);
}
...
char OpCode(char entrada[]){
    ...
    char *caso;
    char *Characternalista;

    //INSERCOES
    if (entrada[0] == 'I'){
        caso = strtok(entrada ," ");
        Characternalista = strtok(NULL ," ");
        fila = inserir(character_searchById(Characternalista) , fila);
    }
    // REMOCOES
    if (entrada[0] == 'R'){
        fila = remover(fila);
