...
void DesempateHeapsort(){
    for (int i = 0; i < contPersonagem; i++){
        if (strcmp(lista[i].hairColour, lista[i + 1].hairColour) == 0){
            for (int j = i + 1; j < contPersonagem; j++){
                if (strcmp(lista[i].hairColour, lista[j].hairColour) == 0){
...
void Heap(int n, int i){
    ...
    if (esq < n){ 
        if(strcmp(lista[dir].hairColour, lista[maior].hairColour) > 0){ 
            maior = esq;
        }
        if(strcmp(lista[dir].hairColour, lista[maior].hairColour) == 0 && strcmp(lista[dir].name, lista[maior].name) > 0){
            maior = esq;
        }
    }
...
void HeapsortParcial(){
    // constroi o heap
    for (int i = contPersonagem / 2 - 1; i >= 0; i--){
        Heap(i, contPersonagem);
    }
    // Retira elementos 1 por 1 no array
    for (int i = contPersonagem - 1; i >= 0; i--){
        swap(0, i);
