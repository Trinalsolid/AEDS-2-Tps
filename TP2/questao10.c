...
void quicksortRec(int esq, int dir){
    ...
    while (i <= j){
        while (strcmp(lista[i].house, pivo.house) < 0){
            i++;
        }
        while (strcmp(lista[j].house, pivo.house) > 0){
            j--;
        }
        if (i <= j){
            swap(i , j);
...
void sort(){
   quicksortRec(0, contPersonagem - 1);
}

void Quick(){
    for (int i = 0; i < contPersonagem; i++){
        if (strcmp(lista[i].house, lista[i + 1].house) == 0){
            for (int j = i + 1; j < contPersonagem; j++){
                if (strcmp(lista[i].house, lista[j].house) == 0){
