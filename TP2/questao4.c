...
bool PesquisaBinaria(char entrada[]){
    ...
    int dir = (contPersonagem - 1), esq = 0, meio;

    while(esq <= dir){
        meio = (esq + dir) / 2;
        if(strcmp(entrada, lista[meio].name) == 0){
            resp = true;
            esq = dir + 1;
            contador++;
        }
        else if(strcmp(entrada, lista[meio].name) > 0){
            esq = meio + 1;
            contador++;
        }
        else{
...
void LerPersonagem(char entradaID[]){
    ...
    char *stringsep;
    char *virgula;
    FILE *caminho = fopen("C:\\Users\\WazX\\Desktop\\aeds2-master\\tps\\entrada e saida\\tp02\\characters.csv","r");
